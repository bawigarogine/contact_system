            ...
            <div class="card w-100">
                <div class="card-header">
                    ...
                    <div class="d-flex align-items-center mt-3 justify-content-end">
                        <a href="/create" class="btn btn-primary rounded-0">Add Contact</a>
                ...
                <div class="card-body table-responsive">
                    <form action="store" method="POST">
                        ...
                        <label>Name</label>
                        <input type="text" name="name" class="form-control my-2" value="{{ old('name') }}" required>
                        @error('name')
                            <p class="text-danger"> {{ $message }} </p>
                        @enderror

                        <label>Company</label>
                        <input type="text" name="company" class="form-control my-2" value="{{ old('company') }}" required>
                        @error('company')
                            <p class="text-danger"> {{ $message }} </p>
                        @enderror

                        <label>Phone</label>
                        <input type="tel" name="phone_num" class="form-control my-2" value="{{ old('phone_num') }}" required>
                        @error('phone_num')
                            <p class="text-danger"> {{ $message }} </p>
                        @enderror

                        <label>Email</label>
                        <input type="email" name="email" class="form-control my-2" value="{{ old('email') }}" required>
                        @error('email')
                            <p class="text-danger"> {{ $message }} </p>
                        @enderror
